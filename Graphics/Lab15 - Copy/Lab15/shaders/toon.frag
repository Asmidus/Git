...
void main()
{
	...
	for (int lightNum = 0; lightNum < totalLights; lightNum++)
	{
		if (lights[lightNum].isEnabled == 1)  //Make sure the light is on
		{
			...
			if (intensity > 0.95)
				colorSum += _color.rgb * I * 1.0;
			else if (intensity > .75)
				colorSum += _color.rgb * I * 0.75;
			else if (intensity > .5)
				colorSum += _color.rgb * I * 0.5;
			else if (intensity > 0.25)
				colorSum += _color.rgb * I * 0.25;
			else
				colorSum += _color.rgb * I * 0.05;
		}
	}
	colorSum = colorSum + globalAmbientLight*color.rgb;  //compute the final color of the fragment
	FragColor = vec4(colorSum.r, colorSum.g, colorSum.b, _color.a);  //use the fragment's original alpha
}